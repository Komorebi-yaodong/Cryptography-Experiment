...
def main():
    ...
    for i in range(times):
        # print(f"第{i}组:")
        # print("变化前iv:", hex(iv)[2:])
        out = sm4_128_encode(iv, k)
        if i == times - 1 and r != 0:
            j = "%032x" % out
            tmp = hex(int(j[0:r], 16) ^ gp[i])[2:]
            while len(tmp) < r:
                tmp = '0' + tmp
        else:
            tmp = hex((out >> (128 - 8 * n)) ^ gp[i])[2:]
            while len(tmp) < 2 * n:
                tmp = '0' + tmp
        ans += tmp
        change = ("%032x" % out)[0:2 * n]
        # print("填充值change:", change)
        iv = ("%032x" % iv)[2 * n:]
        # print("左移后:", iv)
        iv = int((iv + change), 16)
        # print("变化后iv: %032x" % iv)
...
if __name__ == "__main__":
    ...
    graphviz.output_file = 'SM4_OFB.png'
